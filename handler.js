...
const create = async (event, context, callback) => {
  ...
  const linkedServices = body.services.map(service => {
  ...
  const servicePorts = [
    ...
    ...linkedServices.map(service => ({ "protocol": "TCP", "name": service.name, "port": parseInt(service.ports.pop().containerPort)}))
  ];

  const deployment = deployements.post({ body: { "apiVersion": "apps/v1", "kind": "Deployment", "metadata": { "name": `${body.name}-deployment`, "labels": { "app": body.name } }, "spec": { "replicas": 1, "selector": { "matchLabels": { "app": body.name } }, "template": { "metadata": { "labels": { "app": body.name } }, "spec": { "containers": [frontend, ...linkedServices] } } } } })
